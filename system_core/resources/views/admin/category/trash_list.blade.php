                    ...
                    <div class="col-lg col-sm-7 col-md-12 col-12">
                        <ol class="breadcrumb float-right nav_breadcrumb_top_align">
                            ...
                            <li class="breadcrumb-item active">
                                ...
                                @endphp
                                <a href="{{ url('admin/category/'.$parameters) }}"><i class="fa fa-th"></i> {{ ucfirst($parameters) }}</a>
                            </li>
                        ...
                        <div class="card">
                            <div class="card-header bg-white">
                                @php
                                    $parameters = \Request::segment(3);
                                @endphp
                                <i class="fa fa-table"></i> Trash Categories of {{ ucfirst($parameters) }}
                                <a href="{{ url('admin/category/'.$parameters) }}" class="btn btn-success pull-right" title="Back to List">Back to List</a>
                            </div>
                            ...
                            <div class="card-body p-t-25">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="sample_1">
                                    ...
                                    <tbody>
                                        @foreach($trash_categories  as $key => $category)
                                            <tr>
                                                <td>{{ $key+1 }}</td>
                                                <td>{{ $category->name }}</td>
                                                <td>{{ $category->slug }}</td>
                                                ...
                                                <td>
                                                    <a href="{{ url('/admin/category/'.$category->id.'/restore') }}" class="btn btn-info btn-sm"
                                                        onclick="event.preventDefault();
                                                            if (confirm('Are you sure to restore this category ?')) {
                                                                document.getElementById('restore-form-{{ $category->id }}').submit();
                                                            }">
                                                        Restore
                                                    </a>
                                                    <form id="restore-form-{{ $category->id }}" action="{{ url('/admin/category/'.$category->id.'/restore') }}" method="POST" style="display: none;">
                                                        @csrf
                                                    </form>

                                                    <a href="{{ url('/admin/category/'.$category->id.'/destroy') }}" class="btn btn-danger btn-sm"
                                                        onclick="event.preventDefault();
                                                            if (confirm('Are you sure to permanently delete this category ?')) {
                                                                document.getElementById('delete-form-{{ $category->id }}').submit();
                                                            }">
                                                        Delete
                                                    </a>
                                                    <form id="delete-form-{{ $category->id }}" action="{{ url('/admin/category/'.$category->id.'/destroy') }}" method="POST" style="display: none;">
                                                        @csrf
                                                        {{ method_field('DELETE') }}
                                                    </form>
                                                </td>
