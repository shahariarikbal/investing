...
@section('footer_scripts')
    ...
    <script>
        var editor_config = {
            ...
            selector: "textarea.analysisEditor",
                ...
                <div class="row">
                    <div class="col-12 data_tables">
                        ...
                        @if(Session::get('message'))
                            <p class="alert alert-success" style="font-size: 15px;">{{Session::get('message')}}</p>
                        @endif
                        @if ($errors->any())
                        ...
                        <div class="card">
                            <div class="card-header bg-white">
                                ...
                                <a href="{{ url('admin/analysis/trash') }}" style="margin-right:10px;" class="btn btn-danger pull-right trash" title="Trash List">Trash List</a>
                            </div>
                                            ...
                                            <tr>
                                                ...
                                                <td width="25%">
                                                    ...
                                                    @if($analysis->feature_image)
                                                        <img src="{{ asset('/analyses/images/278x180-'.$analysis->feature_image) }}" style="height: 50px;" width="100px">
                                                    @else
                                                        <img src="{{ asset('/analyses/images/default.jpg/') }}" style="height: 80px;" width="100px">
                                                    @endif
                                                ...
                                                <td style="text-align:center" width="15%">
                                                    ...
                                                    @if( $analysis->status == 'inactive' )
                                                        <button href="{{ url('admin/analysis/edit') }}" data-id="{!! $analysis->id !!}" class="edit btn btn-warning tipso m-t-20"  data-background="#ffa000" data-tipso="Click to edit" data-position="bottom">
...
{{--    Add Analysis Modal--}}
    @include('admin.analyses.insert')
{{--    End Blog Modal--}}

{{--    Edit Blog Modal--}}
    @include('admin.analyses.edit')
{{--    Edit Blog Modal End--}}


{{--Analysis Fontend Validation--}}
...
<script>
    ...
    function fetchSubcategory (id, target) {
        ...
        document.getElementById('addAnalysis').addEventListener('click', function (e) {

            add_reset_error();
            addAnalysisValidation();
                ...
                if (error === 0) {
                    ...
                    var height = parseInt(document.getElementById('analysisForm').offsetHeight)
    ...
    // add blog validation script
    var error = 0;
    function editAnalysisValidation() {
  ...
  //edit Analysis Script
        ...
        $.ajax({
                ...
                success(response){
                    ...
                    if(response.feature_image) {
                        $("#prev-img").attr("src", InvestingPartner.app_url + "/analysis/images/278x180-" + response.feature_image)
                    }
                    else {
                        $("#prev-img").attr("src", InvestingPartner.app_url + "/analysis/images/default.jpg")
                    }
                    $("#edit_analysis_seo_title").val(response.seo_optimize.title);
                    $("#edit_analysis_seo_keyword").val(response.seo_optimize.keyword);
                    $("#edit_analysis_seo_description").val(response.seo_optimize.description);
                    //console.log(response.category.parent_id)
    ...
    document.getElementById('submitAnalysis').addEventListener('click', function(e){
        edit_reset_error();
        editAnalysisValidation();
        ...
        //SEO code
        data.append('seo_title', document.getElementById('edit_analysis_seo_title').value)
        data.append('seo_keyword', document.getElementById('edit_analysis_seo_keyword').value)
        data.append('seo_description', document.getElementById('edit_analysis_seo_description').value)
        ...
        if (error === 0) {
            // initialize loading effect
            var height = parseInt(document.getElementById('editAnalysisForm').offsetHeight)

            var padding = (height - 40) / 2

            var loadingStyle = `height: ${height}px; text-align: center; padding: ${padding}px 80px;background: rgba(35, 35, 35, 0.26) none repeat scroll 0% 0%; position: absolute; z-index: 2;width: 100%;top: 0;left:0`

            document.getElementById('edit_form-loading').setAttribute('style', loadingStyle)
            axios.post('/admin/analysis/'+document.getElementById('analysisId').value+'/update', data, settings)
            .then(function (response) {
                    if (response.status === 200) {
                        //location.reload(true)
                        $('#editAnalysisModal').modal('hide');
                        location.reload(true)
                        document.getElementById('edit_form-loading').setAttribute('style', 'display: none')
                        let div = document.createElement("div")
                        div.setAttribute("class", "alert alert-success")
                        div.setAttribute("role", "alert")
                        let txt = document.createTextNode('Analysis has been successfully updated')
                        div.appendChild(txt)
