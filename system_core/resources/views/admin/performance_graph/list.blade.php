    ...
    <script>
        ...
        function validate() {
            ...
            if (document.getElementById('date').value == "") {
                document.getElementById("date_error").innerHTML = "Please select your date";
                error++
            }

            if (document.getElementById('value').value == "") {
                document.getElementById("value_error").innerHTML = "Please type your value";
                error++
            }
        ...
        document.getElementById('saveGraph').addEventListener('click', function(e){
            ...
            .catch(function (error) {
                    ...
                    if(error.response.status === 422) {
                        ...
                        if(error.response.data.value && error.response.data.value.length > 0){
                            document.getElementById("status_error").innerHTML = error.response.data.value[0]
                        }
                    }
                });
        })
        ...
        function edit_graph(el) {
            ...
            axios.get('/admin/graph/edit/'+el.getAttribute('data-id'))
                .then(function (response) {
                    ...
                    document.getElementById("estatus").selectedIndex = response.data.status
        ...
        /*profit checkbox functionality for add_graph*/
        document.getElementById("profit").addEventListener("click", function() {
            if (!this.checked) {
                document.getElementById("value").value = "-" + document.getElementById("value").value.replace('-', '')
            }
            else {
                document.getElementById("value").value = document.getElementById("value").value.replace('-', '')
            }
        ...
        document.getElementById("value").addEventListener("keyup", function() {
            if (this.value.search("-")) {
                ...
                document.getElementById("profit").checked = false
            }
        })
        ...
        <div class="main-bar">
            <div class="row">
                <div class="col-lg col-sm-5 col-md-12 col-12">
                    ...
                    <button type="button" data-toggle="modal" data-target=".addGraphModal" class="btn btn-success pull-right add_graph" title="Add performance graph">
                        Add Performance Graph
                    </button>
                </div>
        ...
        <div class="outer">
            <div class="inner bg-container">
                <div class="row sales_section">
                    ...
                    <div class="col-md-3 media_max_573 cursor-pointer" onclick="location.href = `{{ url('admin/graph/' . strtolower(str_replace('App\\', '', $key))) }}`">
