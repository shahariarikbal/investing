...
@section('title', 'Trashed knowladgebase')
                    ...
                    <div class="col-12 data_tables">
                        ...
                        <div class="card">
                            <div class="card-header bg-white">
                                <i class="fa fa-trash-o"></i>   Knowledgebase Trash List
                            </div>

                            <div class="card-body p-t-25">
                                <a href="{{ url('admin/knowledgebase') }}" class="btn btn-primary pull-right" title="knowledgebase List">Back to Knowledgebases List</a>
                                <div class="">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="sample_1">
                                    <thead>
                                        ...
                                        @foreach($deleted_knowledge as $key=> $knowledge)
                                            <tr>
                                                <td>{{$key+1}}</td>
                                                <td>
                                                    {{ $knowledge->category ? $knowledge->category->name : '' }}
                                                </td>
                                                <td>
                                                    <p>{{ $knowledge->title }}</p>
                                                    @if($knowledge->feature_image)
                                                        <img src="{{ asset('/knowledgebases/images/'.$knowledge->feature_image) }}" style="height: 50px;" width="100px">
                                                    @else
                                                        <img src="{{ asset('/knowledgebases/images/default.jpg') }}" style="height: 80px;" width="100px">
                                                    @endif
                                                    <span class="badge badge-pill badge-{{ $knowledge->status }} float-right x-90">{{ $knowledge->status }}</span>
                                                </td>
                                                <td>{!! substr($knowledge->body,0,100) !!}</td>
                                                <td style="text-align:center">
                                                    <a href="{{ url('admin/knowledgebase/' . $knowledge->id . '/destroy') }}" class="btn btn-danger tipso m-t-20" data-background="#ff8086" data-tipso="Click to destroy" data-position="bottom"
                                                            onclick="event.preventDefault();
                                                            if (confirm('Are You sure you want to destroy this!')) {
                                                                document.getElementById('destroy-form-{{ $knowledge->id }}').submit();
                                                            }">
                                                        <i class="fa fa-trash"></i>
                                                    </a>
                                                    <form id="destroy-form-{{ $knowledge->id }}" action="{{ url('admin/knowledgebase/' . $knowledge->id . '/destroy') }}" method="post" style="display: none;">
                                                        {{ method_field('DELETE') }}
                                                        @csrf
                                                    </form>
                                                    <a href="{{ url('admin/knowledgebase/'.$knowledge->id.'/restore') }}" class="btn btn-raised btn-warning" id="{{$knowledge->id}}">
                                                        <i class="fa fa-refresh"></i>
                                                    </a>
                                                    <form method="get" action="{{ url('admin/knowledgebase/'.$knowledge->id.'/restore') }}" style="display: none">
                                                        @csrf
                                                    </form>
                                                </td>
                                            </tr>
