                    ...
                    <div class="col-12 data_tables">
                        ...
                        <div class="card">
                            <div class="card-header bg-white">
                                <i class="fa fa-trash-o"></i>   Blog Trash List
                            </div>

                            <div class="card-body p-t-25">
                                <a href="{{ url('admin/blogs') }}" class="btn btn-primary pull-right" title="Blog List">Back to Blogs List</a>
                                <div class="">
                                ...
                                <table class="table table-striped table-bordered table-hover" id="sample_1">
                                    <thead>
                                        ...
                                        @foreach($deleted_blogs as $key=> $blog)
                                            <tr>
                                                <td>{{$key+1}}</td>
                                                <td>
                                                ...
                                                <td>
                                                    ...
                                                    @if($blog->feature_image)
                                                        <img src="{{ asset('/blog/images/278x180-'.$blog->feature_image) }}" style="height: 50px;" width="100px">
                                                    @else
                                                        <img src="{{ asset('/blog/images/default.jpg') }}" style="height: 80px;" width="100px">
                                                    @endif
                                                    <span class="badge badge-pill badge-{{ $blog->status }} float-right x-90">{{ $blog->status }}</span>
                                                </td>
                                                <td>{!! substr($blog->body,0,100) !!}</td>
                                                <td style="text-align:center">
                                                    <a href="{{ url('admin/blog/' . $blog->id . '/destroy') }}" class="btn btn-danger tipso m-t-20" data-background="#ff8086" data-tipso="Click to destroy" data-position="bottom"
                                                    ...
                                                    <form id="destroy-form-{{ $blog->id }}" action="{{ url('admin/blog/' . $blog->id . '/destroy') }}" method="post" style="display: none;">
                                                        {{ method_field('DELETE') }}
                                                        @csrf
                                                    </form>
                                                    <a href="{{ url('admin/blog/'.$blog->id.'/restore') }}" class="btn btn-raised btn-warning" id="{{$blog->id}}">
                                                        <i class="fa fa-refresh"></i>
                                                    </a>
                                                    <form method="get" action="{{ url('admin/blog/'.$blog->id.'/restore') }}" style="display: none">
                                                        @csrf
                                                    </form>
                                                </td>
                                            </tr>
