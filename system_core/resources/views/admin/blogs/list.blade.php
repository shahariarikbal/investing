                    ...
                    <div class="col-12 data_tables">
                        ...
                        <div class="card">
                            <div class="card-header bg-white">
                                ...
                                <a href="{{ url('admin/blog/trash') }}" class="btn btn-danger pull-right trash" title="Trash List">Trash List</a>
                            </div>
                                        ...
                                        <tr>
                                            ...
                                            <td width="25%">
                                                ...
                                                @if($blog->feature_image)
                                                    <img src="{{ asset('/blogs/images/278x180-'.$blog->feature_image) }}" style="height: 50px;" width="100px">
                                                @else
                                                    <img src="{{ asset('/blogs/images/default.jpg') }}" style="height: 80px;" width="100px">
                                                @endif
                                                    <span class="badge badge-pill badge-{{ $blog->status }} float-right x-90">{{ $blog->status }}</span>
                                            </td>
                                            <td width="25%">{!! strip_tags(substr($blog->body,0,100)) !!}</td>
                                            <td style="text-align:center" width="15%">
                                                <!--  -->
                                            @if( $blog->status == 'inactive' )
                                                <button href="{{ url('admin/blog/edit') }}" data-id="{!! $blog->id !!}" class="btn btn-warning tipso m-t-20 edit" data-background="#ffa000" data-tipso="Click to edit" data-position="bottom">
...
{{--    Add Blog Modal--}}
    @include('admin/blogs/add_modal')
{{--    End Blog Modal--}}

{{--    Start Edit Blog Modal--}}
    @include('admin/blogs/edit_modal')
{{--    End Edit Blog Modal--}}
...
<script type="text/javascript">
    ...
    //var add_error = 0;
        ...
        // Blog Update Validation Script
        var error = 0;
        function editBlogValidation() {

            if (document.getElementById('ecategory').value == ""){
                document.getElementById("edit_category_error").innerHTML = "Please Select your Category name...";
                error++;
            }
            ...
            if (document.getElementById('estatus').value == "") {
                document.getElementById('edit_status_error').innerHTML = 'Please Select Status';
                error++
            }
...
<script>
    $(document).ready(function () {
        ...
        function fetchSubcategory (id, target) {
        ...
        fetchSubcategory("category", "subcategory")

        fetchSubcategory("ecategory", "esubcategory")
        ...
        document.getElementById('addBlog').addEventListener('click', function(){
            ...
            data.append('subcategory_id', document.getElementById('subcategory').value)
            ...
            if (error === 0) {
                ...
                var height = parseInt(document.getElementById('blogForm').offsetHeight)
                ...
                document.getElementById('blog-loading').setAttribute('style', loadingStyle)    

                axios.post('/admin/blog/store',data, settings)
                .then(function (response) {
                    ...
                    if(response.status === 201) {
                        ...
                        document.getElementById('blog-loading').setAttribute('style', 'display: none')
                        let div = document.createElement("div")
                        div.setAttribute("class", "alert alert-success")
                        div.setAttribute("role", "alert")
                        let txt = document.createTextNode('Blog has been successfully inserted')
                        div.appendChild(txt)
        ...
        $(document).on('click', '.edit', function(e){
            ...
            $.ajax({
                ...
                success(response) {
                    ...
                    $("#eorders").val(response.orders);

                    tinymce.get("ebody").setContent(response.body);
                    $("#blogId").val(response.id);
                    $("#estatus option[value='" + response.status + "']").attr("selected", "selected");
                    if (response.feature_image) {
                        $("#prev-img").attr("src", InvestingPartner.app_url + "/blog/images/278x180-" + response.feature_image)
                    } else {
                        $("#prev-img").attr("src", InvestingPartner.app_url + "/blog/images/default.jpg")
                    }
                    $("#edit_seo_title").val(response.seo_optimize.title);
                    $("#edit_seo_keyword").val(response.seo_optimize.keyword);
                    $("#edit_seo_description").val(response.seo_optimize.description);

                    // $("#image").val(response.featured_image);
                    $("#form_inline_validator").attr("action", "/admin/blog/" + id + "/update");
                    if (response.category.parent_id) {
                        let category = response.category.parent_id
                        let sub_category = response.category_id
                        // sub category
                        // select category
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                // select subcategory
                                $("#esubcategory").val(sub_category)
                            }
                        });

                    } else {
                        let category = response.category_id
                        // category selected
                        $("#ecategory").val(category);
    ...
    /*Update blog with imagfe*/
    document.getElementById('editBlog').addEventListener('click', function(e){
        ...
        data.append('subcategory_id', document.getElementById('esubcategory').value)
        ...
        //SEO code
        data.append('seo_title', document.getElementById('edit_seo_title').value)
        data.append('seo_keyword', document.getElementById('edit_seo_keyword').value)
        data.append('seo_description', document.getElementById('edit_seo_description').value)
        //SEO Code End
        let settings = { headers: { 'content-type': 'multipart/form-data' } }

        if (error === 0) {
            // initialize loading effect
            var height = parseInt(document.getElementById('editBlogForm').offsetHeight)

            var padding = (height - 40) / 2

            var loadingStyle = `height: ${height}px; text-align: center; padding: ${padding}px 80px;background: rgba(35, 35, 35, 0.26) none repeat scroll 0% 0%; position: absolute; z-index: 2;width: 100%;top: 0;left:0`

            document.getElementById('edit_blog-loading').setAttribute('style', loadingStyle) 
            axios.post('/admin/blog/'+document.getElementById('blogId').value+'/update', data, settings)
                .then(function (response) {
                    console.log(response);
                    if(response.status === 200) {
                        ...
                        document.getElementById('edit_blog-loading').setAttribute('style', 'display: none')
                        let div = document.createElement("div")
                        div.setAttribute("class", "alert alert-success")
                        div.setAttribute("role", "alert")
                        let txt = document.createTextNode('Blog has been successfully updated')
                        div.appendChild(txt)
                ...
                .catch(error => {
                    if (error.response.status === 422) {
                        if(error.response.data.status && error.response.data.status.length > 0) {
                            document.getElementById('edit_status_error').innerHTML = 'Please Select Status';
                        }
                    }

                    if (error.response.status === 422) {
                        if (error.response.data.category_id && error.response.data.category_id.length > 0) {
                            document.getElementById('edit_category_error').innerHTML = 'Please Select a Category';
                        }
                    }

                    if (error.response.status === 422) {
                        if (error.response.data.title && error.response.data.title.length > 0) {
                            document.getElementById('edit_title_error').innerHTML = 'Please Write Your Title';
                        }
                    }

                    if (error.response.status === 422) {
                        if (error.response.data.body && error.response.data.body.length > 0) {
                            document.getElementById('edit_body_error').innerHTML = 'Please Write Your Body...';
                        }
                    }
                })
