...
<div style="margin:0.1rem 0 0 0;">
    ...
    @if ($graph->profit)
    <p>You gained {{ $graph->value }}% for {{ $graph->package->name }} of {{ str_replace("App\\", "", $graph->service) }} plan in {{ $graph->month }}, {{ $graph->year }} </p>
    @else
    <p>We are sorry to say you than you lose {{ $graph->value }}% for {{ $graph->package->name }} of {{ str_replace("App\\", "", $graph->service) }} plan in {{ $graph->month }}, {{ $graph->year }} </p>
    @endif
    <div class="row">
        <div class="col-12 text-center social-ions">
            <ul>
                <li>
                    @if ($graph->service === "App\\Copytrade")
                    <a href="{{ url('/member/dashboard/copytrade') }}" class="text-uppercase badge-light">Go To Dashboard</a>
                    @elseif ($graph->service === "App\\FundManagement")
