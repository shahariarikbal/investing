...
<tr>
<td>
<p>Hi, {{ ucfirst($statement->member->first_name) }} {{ ucfirst($statement->member->last_name) }},</p>
<strong>Monthly Trade Statement Invoice Charged Succcessful</strong>

<p>Monthly trade statement Invoice amount {{ $statement->meta['company_share'] }} USD of "{{ $statement->package->name }}" "{{ $service }}" plan charged successful.</p>
...
<td>{{ $invoice->items[0]->name }} <br /> {{ $statement->package->name }}</td>
