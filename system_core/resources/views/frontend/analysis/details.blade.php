...
<!--rating script-->
    ...
    <script>
        ...
        //console.log(`${app_name}.${location.href.split('/')[5].split('~')[0]}.rating`)
        console.log(ratingKey)
        if (ratingKey) {
            let endpoint = `${InvestingPartner.app_url}/analysis/rating/${location.href.split('/')[5]}/${ratingKey}`
            axios.get(endpoint)
                .then(response => {
        ...
        for (var i = 0; i <= labels.length - 1; i++) {
            ...
            document.getElementById(labels[i]).addEventListener('click', function() {
                ...
                let endpoint = `${InvestingPartner.app_url}/analysis/rating/${location.href.split('/')[5].split('~')[0]}`
...
<div class="mt-3">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-3">
                @include('frontend.layouts.includes.analysis-catagory')
                    ...
                    <div>
                        <div class="row article-detials bg-white pb-3" id="initial-content">
                            ...
                            <div class="col-md-12 title-heading">
                                ...
                                <div>
                                    <ul class="social-icon">
                                        <li class="nav-item">
                                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ $analysis->permalink }}" class="nav-link facebook-share" target="_blank"><i class="fab fa-facebook-f facebook-icon"></i></a>
                                        </li>
                                        <li class="nav-item">
                                            <a href="https://twitter.com/intent/tweet?url={{ $analysis->permalink }}?text={{ env('APP_NAME') }}" class="nav-link twitter-share" target="_blank"><i class="fab fa-twitter twitter-icon"></i></a>
                                        </li>
                                        <li class="nav-item">
                                            <a href="https://www.linkedin.com/shareArticle?mini=true&amp;url={{ $analysis->permalink }}" class="nav-link linkedin-share" target="_blank"><i class="fab fa-linkedin-in linkedin-icon"></i></a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="col-md-12 rating">
                                <div>
                                    <ul>
                                        <li><i id="display_rating_1" class="{{ $analysis->average_rating >= 1 ? 'fas' : 'far' }} fa-star" aria-hidden="true"></i></li>
                                        <li><i id="display_rating_2" class="{{ $analysis->average_rating >= 2 ? 'fas' : 'far' }} fa-star" aria-hidden="true"></i></li>
                                        <li><i id="display_rating_3" class="{{ $analysis->average_rating >= 3 ? 'fas' : 'far' }} fa-star" aria-hidden="true"></i></li>
                                        <li><i id="display_rating_4" class="{{ $analysis->average_rating >= 4 ? 'fas' : 'far' }} fa-star" aria-hidden="true"></i></li>
                                        <li><i id="display_rating_5" class="{{ $analysis->average_rating == 5 ? 'fas' : 'far' }} fa-star" aria-hidden="true"></i></li>
                                        <li>
                                            <span id="rating-average">{{ $analysis->average_rating }}.0</span>
                                            (<span id="rating-count">{{ $analysis->rating_count }}</span> ratings)
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="col-md-12 publish-date rating">
                                <ul>
                                    ...
                                    <li>
                                        ...
                                        <span>{{ date('F j Y', strtotime($analysis->created_at)) }}</span>
                                    </li>
                                    {{-- <li>
                                        <span><i class="fas fa-eye text-yellow"></i> 999</span>
                                    </li> --}}
                                    <li>
                                ...
                                <ul class="likes">
                                    ...
                                    <li><span class="badge like-count">{{ $analysis->likes_count }}</span></li>
                                </ul>
                            </div>
                            <div class="col-md-12 analysis-content">
                                <div class="text-center p-3">
                                    <a href="#">
                                        <img class="img-fluid" title="click here to enlarge" data-toggle="modal" data-target="#modal-image" src="{{ asset('/analyses/images/1200x630-'.$analysis->feature_image) }}" alt="">
                                    </a>
                                </div>
                                <p>
                                {!! $analysis->body !!}
								...
								<div class="footer-rating master-review">
									...
									<div>
										<div class="likeButtonArea">
											<div class="mr-2">
												<span style="font-size: 24px; vertical-align: -2px; cursor: pointer; display: none;" data-toggle="tooltip" data-placement="left" title="You already liked this article" class="likeable" id="liked" data-for="unlike" data-id="{{ $analysis->id }}">
													<i class="fas fa-thumbs-up"></i>
												</span>
												<span style="font-size: 24px; vertical-align: -2px; cursor: pointer; display: block;" data-toggle="tooltip" data-placement="left" title="Click to liked this article" class="likeable" id="not-liked" data-for="like" data-id="{{ $analysis->id }}">
													<i class="far fa-thumbs-up"></i>
												</span>
											</div>
											<div class="like-count" id="like-count" data-count="{{ $analysis->likes_count }}">
                            ...
                            <div class="col-md-12 text-center">
                                <div class="card facebook-comment-box">
                                    <div class="card-body">
                                        <div id="fb-comments"  class="fb-comments" data-href="{{ url($analysis->permalink) }}" data-numposts="5"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-12">
            ...
            <div class="col-lg-3">
                ...
                @include('frontend.partials.analyses.related-analyses')
...
<div class="modal fade" tabindex="-1" role="dialog" id="modal-image" aria-hidden="true">
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-body">
				<img class="img-fluid" src="{{ asset('/analyses/images/'.$analysis->feature_image) }}" alt="">
			</div>
		</div>
	</div>
</div>
    ...
    <script>
        ...
        //add liked
        let isLikeClick = false
        document.getElementById('not-liked').addEventListener('click', function () {
            if (isLikeClick) {
                return
            }
            isLikeClick = true
            axios.post(`${InvestingPartner.app_url}/analysis/like/${contentId}`)
                .then(response => {
        ...
        document.getElementById('liked').addEventListener('click', function () {
            axios.delete(`${InvestingPartner.app_url}/analysis/unlike/${contentId}/${localStorage.getItem(`${appName}.${contentId}.like`)}`)
                .then(response => {
                    console.log(response)
