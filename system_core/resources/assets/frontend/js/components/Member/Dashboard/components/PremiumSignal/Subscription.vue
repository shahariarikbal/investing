                    ...
                    <tr>
                        ...
                        <td>
                            <a 
                                ...
                                :class="{ 'btn btn-success custom-btn click-disable' : subscriptions[0].status === 'Active', 'btn btn-danger custom-btn click-disable' : subscriptions[0].status !== 'Active' }">
                                <span>{{ subscriptions[0].status === 'Unpaid' ? 'Pending' : subscriptions[0].status }}</span>
                            </a>
                            <span>
                                <a 
                                    v-if="!subscriptions[0].cancellable && subscriptions[0].cancellation_request_sent"
                                ...
                                <a 
                                    v-if="subscriptions[0].cancellable"
                                    class="btn btn-info btn-sm" 
                                    href="javascript:void(0)"
                                    style="position: relative"
                                    v-popover:info>
                        ...
                        <td>{{ subscriptions[0].plan.settings.billing }}-${{ subscriptions[0].plan.price }}</td>
                        <td class="d-flex justify-content-between" v-if="subscriptions[0].start_at">
                            <span class="service-activate-time" v-if="subscriptions[0].start_at">
                                <i class="fa fa-check"></i> <prity-date  :date="subscriptions[0].start_at" />
                            </span>
                            <span class="service-activate-time" v-if="subscriptions[0].ends_at">
                                <i class="fa fa-ban"></i> <prity-date :date="subscriptions[0].ends_at" />
                            </span>
                        </td>
                        ...
                        <td>
                            ...
                            <a 
                                ...
                                v-if="subscriptions[0].cancellable_from_member_end && !subscriptions[0].cancellation_request_sent"
                                data-toggle="modal" data-target="#subscriptionCancelationModal"
                                @click="cancel(subscriptions[0])">
                                <span>Cancel</span>
                            </a>
                        </td>
                    </tr>
...
<script>
    ...
    export default {
        ...
        methods: {
            getPackages () {
                axios.post('/api/member/premium-signal/subscription')
                    .then(response => {
