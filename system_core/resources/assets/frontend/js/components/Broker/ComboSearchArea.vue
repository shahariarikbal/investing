<template>
    <div>
        <div class="drop-shadow listing-box bg-white mb-3">
            <div class="form-row">
                <div class="col-md-12">
                    <slot name="input">
                        ...
                        <div v-if="inputType==='text'" class="form-group position-relative change-icon">
                            ...
                            <ul v-show="focused && searchSuggestion.length > 0" class="search-suggesation position-absolute">
                                <li v-for="suggesation in searchSuggestion" :key="suggesation.slug" @click="updateSearchParameter(suggesation.slug)">
                ...
                <div class="col-md-12">
                    ...
                    <span class="badge badge-success pointer" @click="viewLess = false" v-if="viewLess">Load More</span>
                    <span class="badge badge-danger pointer" v-else @click="viewLess = true">Collapse</span>
                    <span style="margin-top: 7px;background-color: #ff9800;font-weight: 500;float: right;font-size: 70%;" class="badge badge-warning pointer" v-if="resetable" @click="reset">Reset</span>
                    </div>
                </div>
            </div>            
        </div>
...
export default {
    ...
    watch: {
        input (value) {
            ...
            if (value.length > 0) {
                this.searchSuggestion = this.searchParameter.filter(param => {
