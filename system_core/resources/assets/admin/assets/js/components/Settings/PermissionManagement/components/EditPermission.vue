<template>
    <div class="modal fade show" style="display: block" id="edit-admin-permission-management-modal" role="dialog" aria-labelledby="edit-admin-permission-management-modal">
        
        <div class="modal-dialog" style="position: relative;" id="edit-permission-modal-dialog" role="document">
            <div id="edit-loading" style="display: none">
                <i class="fa fa-spinner fa-pulse fa-5x fa-fw" aria-hidden="true"></i>
            </div>
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h4 class="modal-title text-white">Edit Permission</h4>
                </div>
                <div class="modal-body">
                    <div class="form-group">
                        <label for="name"></label>
                        <input type="text" name="name" id="name" v-model="name" class="form-control" placeholder="Name">
                        <small class="text-danger">{{ edit_nameError }}</small>
                    </div>
                    <div class="form-group">
                        <label for="guard_name">Guard</label>
                        <select class="form-control" name="guard_name" id="guard_name" v-model="guard_name">
                            <option disabled selected>Please select a guard</option>
                            <option value="admin">Admin</option>
                            <option value="member">Member</option>
                        </select>
                        <small class="text-danger">{{ edit_guardError }}</small>
                    </div>
                </div>
                <div class="modal-footer d-flex justify-content-between">
                    <button class="btn btn-danger" data-dismiss="modal" @click="close">Close</button>
                    <button class="btn btn-success" @click="update">Update</button>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    export default {
        data () {
            return {
                permissions: [],
                name: '',
                guard_name: '',
                edit_errorCount: 0,
                edit_nameError: '',
                edit_guardError: '',
            }
        },

        mounted () {
            axios.get(`/xhr/admin/settings/permission-management/edit/${this.$route.params.id}`)
                .then(response => {
                    this.name = response.data.name
                    this.guard_name = response.data.guard_name
                })
                .catch(error => {
                    console.log(error)
                })
        },
        methods: {
            resetError () {
                this.edit_errorCount = 0
                this.edit_nameError = ''
                this.edit_guardError = ''
            },
            validate () {
                if (this.name.length === 0) {
                    this.nameError = 'Permission Name required'
                    this.edit_errorCount++
                }
                if (this.guard_name.length === 0) {
                    this.edit_guardError = 'Permission guard required'
                    this.edit_errorCount++
                }
            },
            close () {
                this.$router.push({ name: 'permissions' })
            },
            update () {
                this.resetError()
                this.validate()

                let height = parseInt(document.getElementById('edit-permission-modal-dialog').offsetHeight)
                let padding = (height - 40) / 2
                let loadingStyle = `height: ${height}px; text-align: center; padding: ${padding}px 80px;background: rgba(35, 35, 35, 0.26) none repeat scroll 0% 0%; position: absolute; z-index: 2;width: 100%;top: 0;left:0`
                document.getElementById('edit-loading').setAttribute('style', loadingStyle)

                let data = {
                    name: this.name,
                    guard_name: this.guard_name,
                }

                if (this.edit_errorCount === 0 ) {
                    axios.post(`/xhr/admin/settings/permission-management/${this.$route.params.id}/update`, data)
                        .then(response => {
                            document.getElementById('edit-loading').setAttribute('style', 'display: none')
                            if (response.status === 200) {
                                EventBus.$emit('permission-update', response.data)
                                this.flash('Permission Update Successfully', 'success')
                                this.close()
                            }
                        })
                        .catch(error => {
                            console.log(error)
                            if (error.response.status === 500) {
                                document.getElementById('edit-loading').setAttribute('style', 'display: none')
                                //console.log('500')
                                this.flash('Internal server error', 'error');
                            }
                            if (error.response.status === 422) {
                                if (error.response.data.name && error.response.data.name.length > 0) {
                                    this.planError = error.response.data.name[0]
                                }
                            }
                            if (error.response.status === 422) {
                                if (error.response.data.guard_name && error.response.data.guard_name.length > 0) {
                                    this.planError = error.response.data.guard_name[0]
                                }
                            }
                        })
                }
            }
        }
    }
</script>