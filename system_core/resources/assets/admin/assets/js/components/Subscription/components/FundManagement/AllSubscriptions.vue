        ...
        <table class="table table-bordered" v-if="subscriptions.length > 0">
            ...
            <tbody v-if="subscriptionPages.length > 0">
                <tr v-for="(subscription, index) in subscriptionPages[currentPage - 1]" :key="subscription.id">
                    ...
                    <td>{{ subscription.plan.duration }} {{ subscription.plan.name }} Package @{{ subscription.plan.details[0].value }}</td>
                    <td>
                    ...
                    <td>
                        ...
                        <template v-if="subscription.security_fund">
                            <b>Security Fund Status: </b>{{ subscription.security_fund.status }} <br />
                            <b>Security Fund: </b>{{ subscription.security_fund.amount | currency }} <br />
                            <b>Active Fund: </b>{{ (subscription.security_fund.amount * subscription.plan.settings.security_deposit) | currency }} <br />
                        </template>
...
<script>
    ...
    export default {
        ...
        components: { DepositModal, TransactionModal, BalanceModal, ApproveSubscriptionModal, MetaTraderInformationModal, DateRangePicker },
        
        created () {
            axios.get('/xhr/admin/subscriptions/fund-management/all-subscripton')
                .then(response => {
                ...
                //Package
                axios.get('/xhr/admin/subscriptions/package/fund-management')
                .then(response => {
                ...
                // members
                axios.get('/xhr/admin/subscriptions/signal/member/fund-management')
                .then(response => {
