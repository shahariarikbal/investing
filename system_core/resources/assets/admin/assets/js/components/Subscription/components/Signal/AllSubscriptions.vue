<template>
    <div class="tab-pane fade show active mt-3" id="pending-deposit" role="tabpanel" aria-labelledby="pending-deposit-tab">
            ...
            <tbody v-if="subscriptionPages.length > 0">
                <tr v-for="(subscription, index) in subscriptionPages[currentPage - 1]" :key="subscription.id">
                    ...
                    <td>
                        ...
                        <br />
                        <button class="btn btn-raised btn-success btn-sm" @click="signalReceivingInformation(subscription.signal_destination)" data-toggle="modal" data-target="#signal-receiving-information-modal">Signal Receiving Information</button>

                    </td>
                    <td>
                        <b>Status: </b>{{ subscription.status }}<br />
                        <span v-if="subscription.payment_method">
                            <b>Payment Method: </b><payment-processor :image="true" :text="false" :data="subscription.payment_method" /><br />
                        </span>
                    ...
                    <td>
                        <button v-if="subscription.status == 'Unpaid'" class="btn btn-raised btn-success btn-sm" @click="approveSubscription(subscription)" data-toggle="modal" data-target="#approve-subscription-modal">Approve Subscription</button>
                    </td>
                </tr>
            </tbody>
            <tbody v-else>
        ...
        <signal-destination-modal v-if="displaySignalDestinationModal" :destinaton="signalDestination" />
...
<script>
    ...
    import SignalDestinationModal from './../Modal/SignalDestinationModal'
    ...
    export default {
        data () {
            return {
                ...
                displaySignalDestinationModal: false,
                signalDestination: {}
            }
        },
        components: { DepositModal, TransactionModal, BalanceModal, ApproveSubscriptionModal, SignalDestinationModal, DateRangePicker },

        created () {
            axios.get('/xhr/admin/subscriptions/signal/all-subscription')
                .then(response => {
                ...
                //Package
                axios.get('/xhr/admin/subscriptions/package/signal')
                .then(response => {
                ...
                // members
                axios.get('/xhr/admin/subscriptions/signal/member/signal')
                .then(response => {
        ...
        methods: {
            ...
            // },
            signalReceivingInformation (signalDestination) {
                this.signalDestination = signalDestination
                this.displaySignalDestinationModal = true
            },
